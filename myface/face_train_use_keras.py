...
class Model:
    ...
    def build_model(self, dataset, nb_classes = 3):
        ...
        #以下代码将顺序添加CNN网络需要的各层，一个add就是一个网络层  32个卷积核 大小(3,3) 卷积模式same 输入张量dataset
        ...
        self.model.add(Conv2D(32, (3, 3)))      # 卷积                    #3 2维卷积层
        self.model.add(BatchNormalization())
        self.model.add(Activation('relu'))                                  #4 激活函数层
        
        self.model.add(MaxPooling2D(pool_size=(2, 2)))                      #5 池化层
        self.model.add(Dropout(0.25))                                       #6 Dropout层 抛弃指定率 FIX
        ...
        self.model.add(Dense(512))                                          #14 Dense层,又被称作全连接层 FIX
        self.model.add(BatchNormalization())
    ...
    def train(self, dataset, nb_epoch = 5 , data_augmentation = True):
        ...
        #使用实时数据提升
            ...
            self.model.fit_generator(
                ...
                steps_per_epoch = dataset.train_images.shape[0],#/self.batch_size,
                epochs = nb_epoch,
                validation_data = (dataset.valid_images, dataset.valid_labels))
    ...
    def face_predict(self, image):    
        ...
        aa=self.model.predict(image,self.batch_size); #，不用
        print('tse--result is ', aa)
        result = (result > 0.90).astype('int32') #大于90%转换为1
        ...
        for popleNum in range(len(result[0])):
            if(1 == result[0][popleNum]):
                break
        ret = popleNum
        #校验，防止所有都检测完成，都没有检测到，popleNum值为最后一个对象值，导致误检测
